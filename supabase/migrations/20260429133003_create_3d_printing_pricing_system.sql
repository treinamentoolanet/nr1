/*
  # Sistema de Precificação para Impressão 3D FDM

  ## Descrição
  Criação completa do banco de dados para o sistema de precificação de impressão 3D FDM.

  ## Tabelas Criadas
  1. `printers` - Impressoras 3D do usuário (valor, consumo, configurações de uso)
  2. `filaments` - Filamentos disponíveis (marca, peso, custo)
  3. `products` - Produtos a serem precificados (associados a impressora e filamento)
  4. `product_parts` - Peças individuais de cada produto (tempo, peso, acabamento)
  5. `marketplaces` - Plataformas de venda com taxas e tarifas
  6. `competitors` - Concorrentes com preços para comparação
  7. `user_settings` - Configurações globais do usuário (energia, depreciação, margens)

  ## Segurança
  - RLS habilitado em todas as tabelas
  - Políticas restritivas: usuários só acessam seus próprios dados
  - product_parts acessíveis via JOIN com products do mesmo usuário
  - competitors acessíveis via JOIN com products do mesmo usuário
*/
